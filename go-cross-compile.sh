...
for PLATFORM in $PLATFORMS; do
  GOOS=${PLATFORM%/*}
  GOARCH=${PLATFORM#*/}
  BIN_FILENAME="release/${OUTPUT}-${GOOS}-${GOARCH}"
  if [[ "${GOOS}" == "windows" ]]; then BIN_FILENAME="${BIN_FILENAME}.exe"; fi
  CMD="GOOS=${GOOS} GOARCH=${GOARCH} go build -ldflags=\"-s -w\" -trimpath -o ${BIN_FILENAME} $@"
  echo "${CMD}"
...
#   CMD="GOOS=linux GOARCH=arm64 go build -ldflags=\"-s -w\" -trimpath  -o ${OUTPUT}-linux-arm64 $@"
#   echo "${CMD}"
#   eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
# fi
...
#     BIN_FILENAME="release/${OUTPUT}-${GOOS}-${GOARCH}${GOARM}"
#     CMD="GOARM=${GOARM} GOOS=${GOOS} GOARCH=${GOARCH} go build -ldflags=\"-s -w\" -trimpath  -o ${BIN_FILENAME} $@"
#     echo "${CMD}"
